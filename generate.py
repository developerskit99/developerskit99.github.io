 ...
 try{{
  ...
  if(["Math Calculators","Finance Calculators","Business Calculators","Student Tools"].includes(cat)){{
   ...
   if(slug==="grade-calculator"){{ var g=window.ToolKit.grade(an,bn||100); setOut(g.pct+"% Grade "+g.letter); return; }}
    ...
    if(slug==="binary-calculator"){{ var bc=window.ToolKit.binaryCalc(tv); if(!bc){{ showErr("Enter format: binary operator binary (e.g. 1010 + 1100). Use +, -, *, /."); return; }} setOut(bc.decA+" ("+bc.a+") "+bc.op+" "+bc.decB+" ("+bc.b+") = "+bc.result+" ("+bc.binResult+")"); return; }}
