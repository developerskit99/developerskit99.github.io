 ...
 try{{
  ...
  if(["Math Calculators","Finance Calculators","Business Calculators","Student Tools"].includes(cat)){{
   ...
   if(slug==="percentage-increase-calculator"){{ setOut(window.ToolKit.pctChange(an,bn).toFixed(2)+"%"); return; }}
    if(slug==="percentage-decrease-calculator"){{ if(!isFinite(an)||!isFinite(bn)){{ showErr("Enter old value (A) and new value (B)."); return; }} var dec=an!==0?((an-bn)/an*100):0; setOut(Math.abs(dec).toFixed(2)+"%"); return; }}
   ...
   if(slug==="grade-calculator"){{ var g=window.ToolKit.grade(an,bn||100); setOut(g.pct+"% Grade "+g.letter); return; }}
    ...
    if(slug==="quadratic-equation-solver"){{ if(!isFinite(an)){{ showErr("Enter coefficient a (A)."); return; }} var qb=isFinite(bn)?bn:0, qc=isFinite(Number(cv))?Number(cv):0; var qres=window.ToolKit.quadratic(an,qb,qc); if(qres.error){{ showErr(qres.error); return; }} setOut("x = "+qres.roots.map(r=>r.toFixed(4)).join(", ")+"\\nDiscriminant: "+qres.disc); return; }}
    if(slug==="binary-calculator"){{ var bexpr=(tv||"").trim(); if(!bexpr){{ showErr("Enter a binary expression (e.g. 1010 + 1100)."); return; }} var bparts=bexpr.match(/^([01]+)\\s*([+\\-*\\/])\\s*([01]+)$/); if(!bparts){{ showErr("Enter format: binary operator binary (e.g. 1010 + 1100). Use +, -, *, /."); return; }} var bA=parseInt(bparts[1],2), bOp=bparts[2], bB=parseInt(bparts[3],2), bRes; if(bOp==="+")bRes=bA+bB; else if(bOp==="-")bRes=bA-bB; else if(bOp==="*")bRes=bA*bB; else if(bOp==="/")bRes=bB!==0?Math.floor(bA/bB):NaN; if(!isFinite(bRes)){{ showErr("Division by zero or invalid operation."); return; }} setOut(bA+" ("+bparts[1]+") "+bOp+" "+bB+" ("+bparts[3]+") = "+bRes+" ("+bRes.toString(2)+")"); return; }}
